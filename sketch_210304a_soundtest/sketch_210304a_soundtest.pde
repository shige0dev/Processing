...
class RhythmMachine implements Instrument {
  ...
  void noteOn(float duration) {
    ...
    for(int j=0;j<6;j=j+1){
     if (toggles[j+1][beat].getBooleanValue()) {
       out.playNote( 0, float(bpm)/120 , new ToneInstrument( params[j][0] ) );
       out.playNote( 0, float(bpm)/120 , new ToneInstrument( params[j][1] ) );
     }
    }
    //metronome
    if (beat%4 == 0) {
